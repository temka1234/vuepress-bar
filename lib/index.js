...
const getDirectories = (source, excludedDirs = []) =>
  readdirSync(source).filter(name => !(name === ".vuepress" || excludedDirs.findIndex(dir => name.startsWith(dir)) >= 0) && isDirectory(join(source, name)));
 ...
 * getName("/some/path/nav.01.how", { navPrefix: "nav", stripNumbers: true }); // how
 */
function getName(path, { navPrefix, stripNumbers, preserveCase } = {}) {
  ...
  name = name.replace("-", " ");
  return preserveCase ? name : titleize(name);
}
...
function side(
  baseDir,
  { stripNumbers, maxLevel, navPrefix, skipEmptySidebar, addReadMeToFirstGroup, mixDirectoriesAndFilesAlphabetically, defaultSidebarCollapsable, excludedDirs },
  relativeDir = "",
  currentLevel = 1
) {
  const fileLinks = getChildren(baseDir, relativeDir, currentLevel > maxLevel);
  if (currentLevel <= maxLevel) {
    getDirectories(join(baseDir, relativeDir), excludedDirs)
      .filter(subDir => !subDir.startsWith(navPrefix))
      .forEach(subDir => {
        const children = side(
          baseDir,
          { stripNumbers, maxLevel, navPrefix, skipEmptySidebar, defaultSidebarCollapsable, excludedDirs },
        ...
        if (children.length > 0 || !skipEmptySidebar) {
          ...
          fileLinks.splice(insertPosition, 0, {
            ...
            ...parseSidebarParameters(subDir, { collapsable: defaultSidebarCollapsable }),
            children
          });
        }
      });
 ...
 */
function parseSidebarParameters(dirname, { collapsable }) {
  const index = dirname.lastIndexOf("--");
  const parameters = collapsable ? { } : { collapsable: false };
  if (index === -1) {
    return parameters;
  }
  ...
  args.forEach(arg => {
    if (collapsable && arg === "nc") {
      parameters.collapsable = false;
    } else if (!collapsable && arg === "c") {
      parameters.collapsable = true;
 ...
 */
function nav(rootDir, { navPrefix, stripNumbers, skipEmptyNavbar, preserveNavTitleCase, excludedDirs }, relativeDir = "/", currentNavLevel = 1) {
  relativeDir = relativeDir.replace(/\\/g, "/");
  const baseDir = join(rootDir, relativeDir);
  const childrenDirs = getDirectories(baseDir, excludedDirs).filter(subDir => subDir.startsWith(navPrefix));
  const options = { navPrefix, stripNumbers, skipEmptyNavbar, preserveNavTitleCase, excludedDirs };
  let result;
  ...
  if (currentNavLevel > 1 && childrenDirs.length === 0) {
    ...
    result = { text: getName(baseDir, { stripNumbers, navPrefix, preserveCase: preserveNavTitleCase }), link: relativeDir + "/" };
    ...
    const items = childrenDirs
      ...
      .filter(Boolean);
    result = currentNavLevel === 1 ? items : { text: getName(baseDir, { stripNumbers, navPrefix, preserveCase: preserveNavTitleCase }), items };
  }
...
function getConfig(rootDirOrOptions, options) {
  ...
  options = {
    ...
    pinyinNav: false,
    defaultSidebarCollapsable: true,
    preserveNavTitleCase: false,
    exludedDirs: [],
